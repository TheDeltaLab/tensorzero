...
use tensorzero_core::stored_inference::LazyRenderedSample;
use tensorzero_inference_types::tool::Tool;
use tensorzero_inference_types::{ContentBlock, FlattenUnknown};
// `GCPSFTConfig` moved to `tensorzero-providers` with the rest of the
// provider-types config (Delta-AI fork).
use tensorzero_providers::provider_types::GCPSFTConfig;
use tensorzero_providers::providers::gcp_vertex_gemini::{
    GCPVertexGeminiContent, GCPVertexGeminiContentPart, GCPVertexGeminiFileURI,
    GCPVertexGeminiPartData, GCPVertexGeminiRole, GCPVertexGeminiSupervisedRow, PROVIDER_TYPE,
...
pub async fn supervised_row_from_rendered_sample<'a>(
    inference: &'a LazyRenderedSample,
) -> Result<GCPVertexGeminiSupervisedRow<'a>, Error> {
    let tools = inference
        .tool_params
        .additional_tools
        .as_ref()
    ...
    Ok(GCPVertexGeminiSupervisedRow::from_parts(
        contents,
        system_instruction,
        tools,
    ))
...
mod tests {
    ...
    async fn test_convert_to_sft_row() {
        ...
        let row = supervised_row_from_rendered_sample(&lazy_inference)
            .await
            .unwrap();
