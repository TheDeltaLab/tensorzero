...
// Modified by Delta-AI under Apache 2.0
// Conversions between `tensorzero-stored-config` grader types and the grader
// types defined in this module. Moved from `tensorzero-core`'s
// `optimization/openai_rft` when providers were extracted to this crate
// (orphan rule: core can no longer implement these).
