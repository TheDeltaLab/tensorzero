...
use tensorzero_error::DelayedError;
...
use tensorzero_inference_types::{
    ...
    ModelInferenceRequestJsonMode, ProviderInferenceResponseStreamInner, TextChunk, ThoughtChunk,
    UnknownChunk,
};
...
use tensorzero_inference_types::{
    ...
    ProviderInferenceResponseArgs, ProviderInferenceResponseChunk, RequestMessage,
...
pub struct GCPVertexGeminiSupervisedRow<'a> {
    // Public for the fine-tuning conversion tests in `tensorzero-optimizers`
    // (the conversion code moved there) (Delta-AI fork).
    pub contents: Vec<GCPVertexGeminiContent<'a>>,
    pub system_instruction: Option<GCPVertexGeminiContent<'a>>,
    #[serde(skip_serializing_if = "Vec::is_empty")]
    pub tools: Vec<GCPVertexGeminiSFTTool<'a>>,
}

impl<'a> GCPVertexGeminiSupervisedRow<'a> {
    /// Constructs a supervised fine-tuning row from its parts.
    /// The conversion from a rendered sample lives in `tensorzero-optimizers`
    /// (it depends on core optimization types) (Delta-AI fork).
    pub fn from_parts(
