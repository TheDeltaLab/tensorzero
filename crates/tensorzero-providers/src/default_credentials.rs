...
//! Lazy per-provider-type default credentials and the `ProviderKind` trait.
//!
//! Extracted from `tensorzero-core`'s `model_table.rs` so the providers crate
//! can load provider credentials without depending on core (Delta-AI fork).

use std::{env, fmt::Display, fs, sync::OnceLock};
...
use tensorzero_inference_types::credentials::{
    Credential, CredentialLocation, CredentialLocationWithFallback,
};
...
pub(crate) fn load_credential_with_fallback(
    location_with_fallback: &CredentialLocationWithFallback,
