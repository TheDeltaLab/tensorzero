...
impl JSONSchema {
    ...
    pub fn compile_background(schema: Value) -> Self {
        ...
        // Best-effort background compilation; plain spawn is fine here (Delta-AI fork).
    ...
    /// Creates a JSONSchema from raw file contents identified by `template_key`.
    ///
    ...
    /// Safe to call outside a tokio runtime.
    pub fn from_str_with_key(template_key: &str, content: &str) -> Result<Self, Error> {
        let schema: Value = serde_json::from_str(content).map_err(|e| {
            Error::new(ErrorDetails::JsonSchema {
                message: format!("Failed to parse JSON Schema `{template_key}`: {e}"),
            })
        })?;

        let validator = jsonschema::validator_for(&schema).map_err(|e| {
            Error::new(ErrorDetails::JsonSchema {
                message: format!("Failed to compile JSON Schema `{template_key}`: {e}"),
            })
        })?;
...
mod tests {
    ...
    async fn test_valid_schema() {
        ...
        let schema = JSONSchema::from_str_with_key(
            "test_schema",
            &std::fs::read_to_string(temp_file.path()).expect("read temp file"),
        )
    ...
    fn test_invalid_schema() {
        ...
        let result = JSONSchema::from_str_with_key("test_schema", invalid_schema);
        assert_eq!(
            result.unwrap_err().to_string(),
            "Failed to compile JSON Schema `test_schema`: \"invalid\" is not valid under any of the schemas listed in the 'anyOf' keyword"
        );
    }
    ...
    fn test_invalid_json_content() {
        ...
        let result = JSONSchema::from_str_with_key("invalid_file.json", "not valid json");
    ...
    fn test_from_path_reuses_compiled() {
        ...
        let compiled_schema = JSONSchema::from_str_with_key("test_schema", schema_json)
            .expect("Failed to load schema");

        // The schema should be pre-compiled (not lazy)
        assert!(
            compiled_schema.compiled.get().is_some(),
            "from_str_with_key should store the compiled validator"
        );
    }
}
