...
async fn test_inference_ttft_ms(function_name: &str) {
    ...
    // The first provider times out, producing a failed model inference row
    // alongside the successful failover row.
    let (failed, successful): (Vec<_>, Vec<_>) =
        model_inferences.iter().partition(|mi| mi.error.is_some());
    assert_eq!(failed.len(), 1, "Expected one failed model inference row");
    assert_eq!(successful.len(), 1);
    println!("model_inferences: {model_inferences:?}");
    assert_eq!(successful[0].model_name, "first_provider_timeout");
    assert_eq!(successful[0].model_provider_name, "second_good");
    // The model inference should have a small TTFT, since the successful model provider
    // is fast
    assert!(
        successful[0].ttft_ms.unwrap() < 100,
...
async fn best_of_n_other_candidate(payload: Value, expected_failed_rows: usize) {
    let response = Client::new()
        .post(get_gateway_endpoint("/inference"))
        .json(&payload)
        .send()
    ...
    // A provider-level timeout also records a failed model inference row for the
    // timed-out candidate; a variant-level timeout drops the in-flight model call
    // instead, so no failed row is written.
    let (failed, successful): (Vec<_>, Vec<_>) =
        model_inferences.iter().partition(|mi| mi.error.is_some());
    assert_eq!(failed.len(), expected_failed_rows);
    assert_eq!(successful.len(), 3);
    assert_eq!(successful[0].inference_id, inference_id);

    let mut model_names: Vec<&str> = successful.iter().map(|mi| mi.model_name.as_str()).collect();
...
async fn best_of_n_judge_timeout(payload: Value) {
    ...
    // leaving us with 2 successful inferences plus a failed row for the judge.
    let (failed, successful): (Vec<_>, Vec<_>) =
        model_inferences.iter().partition(|mi| mi.error.is_some());
    assert_eq!(
        failed.len(),
        1,
        "Expected a failed row for the timed-out judge"
    );
