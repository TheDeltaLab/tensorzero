...
Migrated Synapse request headers also work as `x-tensorzero-*` (`provider`, `fallback`, `request-id`, `cache`, `stream-aggregate`, `request-profile`, `response-style`). `x-synapse-*` is still accepted; when both prefixes are set, `x-tensorzero-*` wins. Responses include both `x-tensorzero-*` and `x-synapse-*` (`request-id`, `served-by`, `fallback-count`).
...
You can also send `x-tensorzero-episode-id` (or `x-tensorzero-episodes-id`) on the request. The body field wins if both are set. Reusing the same episode id groups multiple inferences on the Episodes page.
...
Behaves the same as the [`namespace`](/gateway/api-reference/inference#namespace) parameter in the native API.
...
You can also send `x-tensorzero-tags` with comma-separated `key=value` pairs (bare keys become `true`), for example `env=prod,team=ml,canary`. Body tags win on key conflicts. The Inferences and Models pages can filter and aggregate by these tags.
...
When using the OpenAI client SDKs, pass these as `extra_headers`.
