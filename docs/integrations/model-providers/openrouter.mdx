...
[Jev](https://openrouter.ai/typesafe/jev-1.13) is TypeSafe's System One model. It does not generate chat text. Send a `state` and typed `questions`; OpenRouter returns a yes/no probability (`noul`), a `choice`, or a `score`.

TensorZero forwards `POST /v1/systemone` (and `/openai/v1/systemone`) to OpenRouter's System One API. Set `OPENROUTER_API_KEY`. Bare names `jev` and `jev-latest` route to `~typesafe/jev-latest`. `jev-1.13` routes to `typesafe/jev-1.13`.
